...
var input = require('fs').readFileSync(path.resolve('./10. DP_2/03.2193.txt'), 'utf8').toString().trim().split(' ');
...
for (ix = 2; ix <= num; ix++) {
    ...
    memo[ix][0] = add(memo[ix - 1][0], memo[ix - 1][1]);
    memo[ix][1] = memo[ix - 1][0];
}

console.log(add(memo[num][0], memo[num][1]));
